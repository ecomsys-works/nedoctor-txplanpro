...
export default function Pricing() {
    ...
    return (
        <section id="#pricing" className="pricing gsap-up my-container pt-[5px] pb-[50px] 2xl:pb-[50px] 3xl:pb-[80px] mb-[70px] cover-gradient overflow-visible text-white rounded-[20px] 
        sm:mb-[100px] sm:rounded-[30px] mdd:mb-[95px] 2xl:mb-[200px] 2xl:rounded-[80px] 3xl:rounded-[60px] ">

            <h2 className={` text-white tracking-[-0.06em] uppercase text-center                        
             ${t('lang') === 'ru' ? "pb-[32px] pt-[40px] leading-[1]  font-bold text-[40px] sm:text-[78px] sm:pb-[18px] 2xl:text-[120px] 2xl:pt-[28px] 2xl:pb-[33px] 3xl:pt-[58px] 3xl:pb-[40px]" : 
                "pb-[32px] pt-[40px] leading-[1] text-[40px] sm:text-[78px] sm:pb-[18px] 2xl:pb-[10px] 2xl:text-[160px] 3xl:pt-[75px] 3xl:pb-[0px] font-semibold font-anek"}
            `}>{title}</h2>

            {/* DESKTOP: Верхние 3 вертикальные карточки */}
            <div className={`hidden 3xl:grid grid-cols-[523fr_510fr_510fr] gap-x-[22px] mb-[60px] 3xl:mr-[10px] 
                  ${t('lang') === 'ru' ? "" : "2xl:-mt-[20px]"}
                `}>
                {cards.slice(1, 4).map((card, i) => {
                    const { left, right } = splitIncluded(card);                                
                    return (
                        <div key={i} className="card-3 rounded-[28px] p-[15px] flex flex-col relative h-full">
                            {/* Верхний блок */}
                            <div className="bg-white text-black rounded-[28px] p-[22px] relative">
                                ...
                                {card.monthLimits && (
                                    <div className="text-black ">
                                        <h4 className="mb-[16px] text-[24px] leading-[1.1] tracking-[-0.06em] font-semibold">{card.monthLimitsTitle}</h4>
                            ...
                            <div className="flex flex-col h-full justify-between">
                                ...
                                <div className="px-[26px]">
                                    {card.planPrice && (
                                        <p className="mb-[30px] pt-[15px] text-[18px] leading-[1.3] tracking-[-0.04em] font-light">{card.planPrice}</p>
                                    )}
                                    {left.length > 0 && (
                                    ...
                                    <div className={`grid grid-cols-1 
                                     ${t('lang') === 'ru' ? `3xl:grid-cols-${right.length > 0 ? "2" : "1"}` : `3xl:grid-cols-[46%_54%] `}                                        
                                        gap-[8px] mb-[30px]`}>
                                        {left.length > 0 && (
                                            <ul className="space-y-1">
                                                {left.map((item, idx) => (
                                                    <li key={idx} className="flex items-start gap-[5px]">
                                                        <span className="mt-[3px] w-[17px] h-[17px] bg-white rounded-full flex justify-center items-center text-white font-light">
                                                            <svg className="w-[9px] h-[7px] text-black">
                                                                <use href="./icons/sprite/sprite.svg#check"></use>
                                                            </svg>
                                                        </span>
                                                        <span className="text-nowrap text-[18px] leading-[1.3] tracking-[-0.04em] font-normal">{item}</span>
                                                    </li>
                                                ))}
                                            </ul>
                                        )}
                                        ...
                                        {right.length > 0 && (
                                            <ul className="space-y-1 ">
                                                {right.length > 0 && card.limitsTitle && (
                                                    <>
                                                        <h4 className="text-[24px] font-semibold mb-[8px] ">{card.limitsTitle}</h4>
                                                        {right.map((item, idx) => (
                                                            <li key={idx} className="flex items-start gap-[5px]">
            ...
            <div className="hidden 3xl:flex ">
                {(() => {
                    ...
                    const { left, right } = splitIncluded(card);
                    return (
                        <div className="card-long rounded-[28px] w-full h-full grid grid-cols-3 gap-[40px] relative ">
                            {/* Левый блок */}
                            <div className="p-[12px]">
                                <div className="bg-white text-black rounded-[28px] p-[22px] relative">
                                    <div className="min-h-[115px] pb-[12px] flex justify-between items-start">
                                        <div className="space-y-1">
                                            {card.title && (<h3 className="text-[40px] font-semibold leading-[1] tracking-[-0.06em]">{card.title}</h3>)}
                                            {card.description && (<p className="text-[18px] font-normal leading-[1.3] tracking-[-0.04em]">{card.description}</p>)}
                                        </div>
                                    ...
                                    {card.monthLimits && (
                                        <div className=" text-black ">
                                            <h4 className="mb-[15px] text-[24px] leading-[1.1] tracking-[-0.06em] font-semibold">{card.monthLimitsTitle}</h4>
                                            <ul className="text-[13px] space-y-[4px]">
                                                {card.monthLimits.map((item, idx) => (
                                                    <li key={idx} className="flex items-start gap-[9px]">
                                                        <span className="mt-[3px] w-[17px] h-[17px] bg-black rounded-full flex justify-center items-center text-black">
                                        ...
                                        <ul className="space-y-1">
                                            {left.map((item, idx) => (
                                                <li key={idx} className="flex items-start gap-[9px]">
                                                    ...
                                                    <span className="text-[18px] leading-[1.3] tracking-[-0.04em] font-normal max-w-[285px]">{item}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                            ...
                            {/* Правый блок */}
                            {right.length > 0 && (
                                <div className="py-[15px] px-[18px] flex flex-col justify-between relative">
                                    <div className="block">
                                        {card.limitsTitle && (
                                    ...
                                    {card.whiteLabel && (
                                        <span className="inline-flex h-[35px] items-center justify-center absolute top-[13px] right-[12px] bg-white 
                                      text-black rounded-[100px] px-[17px] text-[18px] leading-[1.3] tracking-[-0.04em]  ">
                                            {card.whiteLabel}
                                        </span>
                                    )}
            ...
            <div className="grid grid-cols-1 mdd:grid-cols-2 gap-y-10 gap-x-5 3xl:hidden">
                {cards.map((card, i) => {
                    const { left, right } = splitIncluded(card);
                    return (
                        <div key={i} className={`card-3 rounded-[18px] 2xl:rounded-[28px] p-[10px] 2xl:p-[15px] flex flex-col relative h-full justify-between`}>
                            <div className="flex flex-col">
                                <div className="bg-white text-black rounded-[18px] p-[12px] xl:p-[20px] relative">
                                    <div className="flex justify-between items-start mdd:min-h-[115px]">
                                        <div className="space-y-1">
